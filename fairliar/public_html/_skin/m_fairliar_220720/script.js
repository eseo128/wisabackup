...
function toggle_nav_cate(name) {
	var cnt = $('nav.lnb #'+name);
	if(cnt.css('display') == 'block') {
		cnt.hide();
		$('nav.lnb .cate_wrap').show();
	}
	else {
		cnt.show();
		$('nav.lnb .cate_wrap').hide();
	}
}
function toggle_nav_cate_close() {
	$('nav.lnb .depth_wrap').hide();
	$('nav.lnb .cate_wrap').show();
}

/* 토글 */
function toggle_view(selector, obj, opt){
	var search = $('#'+selector+'');
	var obj = $(obj);
...
$(window).scroll(function(){
	...
	if(topBnr.css('display') == 'block') {
		if(y > topBnr.outerHeight()) {
			...
			$('nav.lnb').removeClass('bnr_show');
		}else {
			$('header .fix').removeClass('fixed');
			$('nav.lnb').addClass('bnr_show');
		}
	}else {
		if(y > 0) {
			$('header .fix').addClass('fixed');
...
$(document).ready(function(){
	...
	if(cookiedata.indexOf('toughcookie=done') > 0 ){
		...
		if(document.getElementById('topbnr')) document.getElementById('topbnr').style.display = 'block';
		$('nav.lnb').addClass('bnr_show');
	}
});
function todayclose(){
...
// 상단 카테고리 자동 노출
$(document).ready(function(){
	$('.lnb .child .big').click(function() {
		var num = $('.lnb .child .big').index(this);
		$('.lnb .child .big').each(function(no){
			if (no == num){
				if($(this).next('.mid').css('display') == 'block') {
					$(this).removeClass('active');
					$(this).next('.mid').slideUp();
				} else {
					$(this).addClass('active');
					$(this).next('.mid').slideDown();
				}
			} else {
				$(this).removeClass('active');
				$(this).next('.mid').slideUp();
...
$(document).ready(function(){
	$('nav.lnb .cate_wrap > ul > li.child > span').click(function() {
		var num = $('nav.lnb .cate_wrap > ul > li.child > span').index(this);
		$('nav.lnb .cate_wrap > ul > li.child > span').each(function(no){
			if (no == num){
				if($(this).next('.mid').css('display') == 'block') {
					$(this).removeClass('active');
					$(this).next('.mid').slideUp();
...
//상단 카테고리 내 중분류 카테고리 자동 노출 (OUTLET)
$(document).ready(function(){
	$('nav.lnb .depth_wrap .depth .mid .child2 > span').click(function() {
		var num = $('nav.lnb .depth_wrap .depth .mid .child2 > span').index(this);
		$('nav.lnb .depth_wrap .depth .mid .child2 > span').each(function(no){
			if (no == num){
				if($(this).next('.mid2').css('display') == 'block') {
					$(this).removeClass('active');
					$(this).next('.mid2').slideUp();
				} else {
					$(this).addClass('active');
					$(this).next('.mid2').slideDown();
				}
			} else {
				$(this).removeClass('active');
				$(this).next('.mid2').slideUp();
